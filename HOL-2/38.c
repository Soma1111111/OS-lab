...
Description : FIFO Communication (Two-Way): Write two programs to establish communication through FIFO
...
int main()
{
    ...
    {
    case 1:
        fd1 = open("myFifo", O_CREAT|O_WRONLY, 0666);
        write(fd1, "Hello from 1 to 2\n", sizeof("Hello from 1 to 2\n"));
        close(fd1);

        fd2 = open("yourFifo", O_CREAT|O_RDONLY, 0666);
        ...
        break;
    case 2:
        fd2 = open("myFifo", O_CREAT|O_RDONLY, 0666);
        read(fd2, buf, sizeof(buf));
        printf("Message from writer: %s", buf);
        close(fd2);
        fd1 = open("yourFifo", O_CREAT|O_WRONLY, 0666);
